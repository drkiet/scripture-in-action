...
public class CollectingTextRSV {
	public static final Logger logger = LoggerFactory.getLogger(CollectingTextRSV.class);
	private static final String RSV_CONTENT_URL = "https://www.ewtn.com/devotionals/biblesearch.asp";
	...
	public void should_parse_rsv_book() {
		...
		Path path = Paths.get(System.getProperty("sia.bible.nab.folder"), "temp.txt");

		List<String> allLines = TestUtils.readAllLines(path);

		int idx = 0;
		String text = collectingSteps.makeRsvContent(NewAmerican.NAMES_OF_ALL_BOOKS[idx], allLines);
	...
	public void rsv_chapters_verses_agrees_with_constants() {
		...
		for (int bookIdx = 0; bookIdx < NewAmerican.NUMBER_OF_ALL_BOOKS; bookIdx++) {
			...
			int numChapters = Math.min(versesByChapters.size(), NewAmerican.NUMBER_OF_CHAPTERS_BY_BOOK_NAME[bookIdx]);
